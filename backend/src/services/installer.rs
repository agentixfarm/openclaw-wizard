...
impl InstallerService {
    ...
    async fn install_node(tx: &mpsc::Sender<InstallProgress>) -> Result<()> {
        if let Ok(Some(version)) = SafeCommand::check_node_version()
            && Self::parse_node_major(&version).is_some_and(|major| major >= Self::MIN_NODE_MAJOR)
        {
            let _ = tx
            ...
            _ => {
                let _ = tx
                    .send(InstallProgress {
                        ...
                        status: "failed".into(),
                        message: format!(
                            "Unsupported OS for automatic Node.js installation: {}",
                            os
                        ),
    ...
    async fn install_node_macos(tx: &mpsc::Sender<InstallProgress>) -> Result<()> {
        ...
        let pkg_url = format!(
            "https://nodejs.org/dist/{}/node-{}.pkg",
    ...
    async fn install_openclaw(tx: &mpsc::Sender<InstallProgress>) -> Result<()> {
        ...
        let _ = tx
            .send(InstallProgress {
                ...
                output_line: Some("$ npm install -g openclaw".to_string()),
                progress_pct: Some(10),
                ..Default::default()
            })
            .await;
    ...
    async fn verify_installation(tx: &mpsc::Sender<InstallProgress>) -> Result<()> {
        ...
        if let Err(e) = Self::deploy_config(tx).await {
            let _ = tx
                .send(InstallProgress {
                    stage: "verify".into(),
                    status: "failed".into(),
                    message: "Configuration deployment failed".into(),
                    error: Some(format!("Could not configure OpenClaw: {}", e)),
                    ..Default::default()
                })
                .await;
            anyhow::bail!("Config deployment failed: {}", e);
        }
                ...
                if output.stderr.contains("already") || output.stdout.contains("already") =>
            {
                ...
                let _ = tx
                    .send(InstallProgress {
                        stage: "verify".into(),
                        status: "running".into(),
                        message: "✓ Gateway service already installed".into(),
                        progress_pct: Some(80),
                        ..Default::default()
                    })
                    .await;
            ...
            Ok(output) => {
                let _ = tx
                    .send(InstallProgress {
                        stage: "verify".into(),
                        status: "failed".into(),
                        message: "Gateway service installation failed".into(),
                        error: Some(format!("Install error: {}", output.stderr.trim())),
                        ..Default::default()
                    })
                    .await;
                anyhow::bail!("Gateway service install failed");
            }
            Err(e) => {
                let _ = tx
                    .send(InstallProgress {
                        stage: "verify".into(),
                        status: "failed".into(),
                        message: "Could not install gateway service".into(),
                        error: Some(format!("Command error: {}", e)),
                        ..Default::default()
                    })
                    .await;
                anyhow::bail!("Could not run gateway install command");
            }
        }
            ...
            Ok(output) if output.exit_code == 0 => {
                let _ = tx
                    .send(InstallProgress {
                        stage: "verify".into(),
                        status: "running".into(),
                        message: "Gateway start command succeeded".into(),
                        progress_pct: Some(92),
                        ..Default::default()
                    })
                    .await;
            }
            Ok(output) => {
                let _ = tx
                    .send(InstallProgress {
                        stage: "verify".into(),
                        status: "failed".into(),
                        message: "Failed to start gateway".into(),
                        error: Some(format!("Gateway start failed: {}", output.stderr.trim())),
                        ..Default::default()
                    })
                    .await;
                anyhow::bail!("Gateway start command failed");
            }
            Err(e) => {
                let _ = tx
                    .send(InstallProgress {
                        ...
                        status: "failed".into(),
                        message: "Failed to start gateway".into(),
                        error: Some(format!("Could not run gateway start: {}", e)),
                        ..Default::default()
                    })
                    .await;
                anyhow::bail!("Could not run gateway start command");
            }
        }

        // Wait for gateway to warm up (LaunchAgents can take a few seconds)
        let _ = tx
            .send(InstallProgress {
                stage: "verify".into(),
                status: "running".into(),
                message: "Waiting for gateway to warm up...".into(),
                progress_pct: Some(94),
                ..Default::default()
            })
            .await;
        ...
        let _ = tx
            .send(InstallProgress {
                stage: "verify".into(),
                status: "running".into(),
                message: "Verifying gateway health...".into(),
                progress_pct: Some(96),
                ..Default::default()
            })
            .await;

        match SafeCommand::run("openclaw", &["gateway", "status"]) {
            Ok(output) if output.exit_code == 0 && output.stdout.contains("RPC probe: ok") => {
                let _ = tx
                    .send(InstallProgress {
                        stage: "verify".into(),
                        status: "running".into(),
                        message: "✓ Gateway is healthy and reachable".into(),
                        progress_pct: Some(98),
                        ..Default::default()
                    })
                    .await;
            }
            Ok(output) if output.stdout.contains("running") => {
                // Gateway process is running but RPC probe might still be warming up
                let _ = tx
                    .send(InstallProgress {
                        stage: "verify".into(),
                        status: "running".into(),
                        message: "✓ Gateway is running (warming up)".into(),
                        progress_pct: Some(98),
                        ..Default::default()
                    })
                    .await;
            ...
            _ => {
                let _ = tx
                    .send(InstallProgress {
                        stage: "verify".into(),
                        status: "failed".into(),
                        message: "Gateway failed health check".into(),
                        error: Some(
                            "Gateway not responding. Try restarting from the dashboard.".into(),
                        ),
                        ..Default::default()
                    })
                    .await;
                anyhow::bail!("Gateway health check failed");
            }
        }
        ...
        let _ = tx
            .send(InstallProgress {
                stage: "verify".into(),
                status: "completed".into(),
                message: "Setup complete! Gateway is running.".into(),
                progress_pct: Some(100),
                ..Default::default()
            })
            .await;
    ...
    async fn deploy_config(tx: &mpsc::Sender<InstallProgress>) -> Result<()> {
        let wizard_config_dir =
            Platform::config_dir().context("Failed to determine wizard config directory")?;
        ...
        // Read wizard config
        let config_bytes = tokio::fs::read(&wizard_config_path)
            .await
            .context("Failed to read wizard config")?;
        let config: serde_json::Value =
            serde_json::from_slice(&config_bytes).context("Failed to parse wizard config")?;
        ...
        match provider {
            ...
            "cloudflare" => {
                args.extend([
                    "--auth-choice".into(),
                    "cloudflare-ai-gateway-api-key".into(),
                ]);
                if !api_key.is_empty() {
                ...
                {
                    args.extend([
                        "--cloudflare-ai-gateway-account-id".into(),
                        account_id.into(),
                    ]);
                }
                ...
                {
                    args.extend([
                        "--cloudflare-ai-gateway-gateway-id".into(),
                        gateway_id.into(),
                    ]);
                }
            }
